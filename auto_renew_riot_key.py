        ...
        with sync_playwright() as p:
            try:
                context = p.chromium.launch_persistent_context(
                    ...
                    channel="chrome",
                    headless=False,
                    no_viewport=True,  # 🔥 ESTA ES LA CLAVE PARA PANTALLA COMPLETA REAL
                ...
                if "lock" in error_str or "in use" in error_str:
                    ...
                    print("[RENOVADOR] 💡 Causa probable 1: Tienes una ventana de Chrome abierta usando este perfil.")
                    print("[RENOVADOR] 💡 Causa probable 2: OneDrive u otro proceso ha bloqueado la carpeta.")
                raise ctx_err
            ...
            if minutes_left < 30:
                print("[RENOVADOR] ⚠️ Faltan menos de 30 minutos. Pausando peticiones y renovando...")

                if api_event:
                    api_event.clear()

                try:
                    # 1. EXTRAER CLAVE VIEJA DE FORMA NATIVA (CON SCROLL HUMANO)
                    clave_vieja = None
                    try:
                        show_btn = page.locator("button:has-text('Show')").first
                        if show_btn.is_visible(timeout=2000):
                            show_btn.scroll_into_view_if_needed()
                            page.mouse.wheel(0, -250)  # Sube la rueda un poco
                    ...
                    if captcha_checkbox.is_visible(timeout=5000):
                        if captcha_checkbox.get_attribute("aria-checked") != "true":
                            ...
                            print("[RENOVADOR] 🤖 Clic en el reCAPTCHA realizado. Esperando validación...")
                            ...
                            try:
                                ...
                                print("⚠️ [ATENCIÓN] El reCAPTCHA pide resolver imágenes o está tardando. Tienes 60s para hacerlo manualmente.")
                            ...
                            print("[RENOVADOR] ✅ El reCAPTCHA ya estaba marcado.")
                    # --- FIN DE GESTIÓN DE CAPTCHA ---

                    print("[RENOVADOR] 🔄 Buscando botón de Regenerar...")
                    boton_regenerar = page.locator("[name='confirm_action']").first

                    # 3. CENTRAR BOTÓN DE REGENERAR
                    boton_regenerar.scroll_into_view_if_needed()
                    # A veces queda muy abajo, bajamos un poco más la rueda
                    ...
                    # COMPROBACIÓN DE ERROR "INVALID CAPTCHA"
                    if page.locator("text=Invalid captcha!").is_visible() or page.locator("text=Invalid captcha").is_visible():
                        print("[RENOVADOR] ❌ ERROR: Riot ha rechazado el captcha.")
                        print("[RENOVADOR] 🛑 VENTANA CONGELADA DURANTE 30 SEGUNDOS. Abre la consola (F12) rápido para investigar...")
                    ...
                    try:
                        ...
                        if show_btn_new.is_visible(timeout=2000):
                            ...
                            # Usamos un bucle nativo de Playwright para esperar a que cambie el estado sin JS
                            page.locator("#apikey").wait_for(state="visible", timeout=5000)
                    except Exception:
                        pass  # Si no encuentra el botón o falla, continuamos intentando extraerlo

                    new_key = page.locator("#apikey").get_attribute("value")

                    if not new_key:
                        raise Exception("No se pudo extraer el atributo 'value' del input #apikey. El valor devuelto fue nulo/vacío.")

                    if clave_vieja and new_key == clave_vieja:
                        print(f"[RENOVADOR] ❌ ERROR: La página generó la misma API Key.")
                        context.close()
                        return new_key, 10
                    ...
                    if new_key and new_key.startswith("RGAPI-"):
                        try:
                            ...
                            print("[RENOVADOR] ✅ Nueva RIOT_API_KEY generada y guardada en .env.")
                        ...
                        raise Exception(f"La clave generada es inválida o el formato cambió. Extracción devolvió: {new_key}")
                ...
                print(f"[RENOVADOR] ☕ Sobran 30 minutos o más. Calentando perfil para generar confianza...")
                ...
                print(f"[RENOVADOR] ℹ️ Navegador cerrado. Siguiente revisión programada en aprox {espera_real} min reales.")
                context.close()
